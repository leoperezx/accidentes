...
def main():
    ...
    options = st.multiselect('¿Qué automóviles desea consultar?',
        ['AUTOMOVIL','BICICLETA','BUS','BUSETA','CAMION','CAMIONETA',
        'CAMPERO','CICLISTA','MAQUINARIA','MICROBUS','MOTO','MOTOCARRO',
        'NO APLICA','TRACTO CAMION','TRICICLO','VOLQUETA'],key="selection")
    
    if st.button("Crear mapa"):
        fn.generarMapa(df,options)
    else: 
